...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (checkReservationBtn && reservationCheckInput && reservationResultDiv) {
        checkReservationBtn.addEventListener('click', () => {
            ...
            if (!query) {
                reservationResultDiv.innerHTML = '<div class="alert alert-warning mt-2">Please enter a VIN or license plate.</div>';
                return;
            }
            ...
            if (match) {
                reservationResultDiv.innerHTML = `
                    <div class="alert alert-danger mt-2">
                        <i class="fa-solid fa-exclamation-circle"></i> <strong>This vehicle has already been reserved!</strong><br>
                        <span class="small">Reserved by: ${match.buyer || 'Unknown'} • Reserved on: ${new Date(match.date).toLocaleDateString('en-US')}</span>
                    </div>
                `;
            } else {
                reservationResultDiv.innerHTML = `
                    <div class="alert alert-success mt-2">
                        <i class="fa-solid fa-check-circle"></i> <strong>No reservations found.</strong> You can continue listing this vehicle.
                    </div>
                `;
            }
        });
